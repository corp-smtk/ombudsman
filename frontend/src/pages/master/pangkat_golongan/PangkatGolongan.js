...
const PangkatGolongan = () => {
    ...
    const viewData = async () => {
        ...
        await new ServiceApi().getPangkat(param).then(x => {
            setDataCount(x.data.total_data);
            setListPangkat(x.data.data);
    ...
    function handlePerPage(e) {
        ...
        new ServiceApi().getPangkat(param).then(x => {
            setListPangkat(x.data.data);
    ...
    return (
        <div className='main-animation'>
            <div className="d-flex flex-row justify-content-between align-items-center">
                <div>
                    <h3 className="content-title">Pangkat & Golongan</h3>
                </div>
                <div>
                    <Link className="content-link" to={{ pathname: `/master/pangkat_golongan/tambah` }}><Button className="content-button d-flex flex-row align-items-center"><AiIcons.AiOutlinePlus style={style} />&nbsp; Tambah Data</Button></Link>
                </div>
            </div>
                    ...
                    <div id="content-table" className="content-table">
                        <div className="scroll-me">
                            <table className="table table-hover">
                                ...
                                <tbody>
                                    {
                                        !_.isEmpty(listPangkat) ?
                                            listPangkat.map((x, key) => {
                                                return (
                                                    <tr key={key}>
                            ...
                            <div>
                                {
                                    !_.isEmpty(listPangkat) ?
                                        <>
                                            Menampilkan data {((currentPage * perPage) - perPage) + 1} - {listPangkat.length == perPage ? (currentPage * perPage) : (currentPage * perPage) - (perPage - listPangkat.length)} dari {dataCount} data
                                        </>
                                        :
                                        <>
