...
const ManajemenUser = () => {
    ...
    const viewData = async () => {
        ...
        await new ServiceApi().getManajemenUser(param).then(x => {
            setDataCount(x.data.total_data);
            setListUser(x.data.data);
    ...
    function handlePerPage(e) {
        ...
        new ServiceApi().getManajemenUser(param).then(x => {
            setListUser(x.data.data);
    ...
    const deleteData = (x) => {
        ...
        Swal.fire({
            ...
            if (response.isConfirmed) {
                new ServiceApi().deleteManajemenUser(data)
                    .then(response => {
                        Swal.fire({
                            title: 'Sukses!',
    ...
    return (
        <div className='main-animation'>
            <div className="d-flex flex-row justify-content-between align-items-center">
                <div>
                    <h3 className="content-title">Manajemen User</h3>
                </div>
                <div>
                    <Link className="content-link" to={{ pathname: `/master/manajemen_user/tambah` }}><Button className="content-button d-flex flex-row align-items-center"><AiIcons.AiOutlinePlus style={style} />&nbsp; Tambah Pengguna</Button></Link>
                </div>
            </div>
                            ...
                            <table className="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="table-title" scope="col">Nama Pengguna</th>
                                        <th className="table-title" scope="col">Role</th>
                                        <th className="table-title" scope="col">Akses</th>
                            ...
                            <div>
                                {
                                    !_.isEmpty(listUser) ?
                                        <>
                                            Menampilkan data {((currentPage * perPage) - perPage) + 1} - {listUser.length == perPage ? (currentPage * perPage) : (currentPage * perPage) - (perPage - listUser.length)} dari {dataCount} data
                                        </>
                                        :
                                        <>
