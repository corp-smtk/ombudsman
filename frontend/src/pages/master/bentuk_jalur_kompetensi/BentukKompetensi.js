...
const BentukKompetensi = () => {
    ...
    const viewData = async () => {
        ...
        await new ServiceApi().getBentukKompetensi(param).then(x => {
            setDataCount(x.data.total_data);
            setListBentuk(x.data.data);
    ...
    function handlePerPage(e) {
        ...
        new ServiceApi().getBentukKompetensi(param).then(x => {
            setListBentuk(x.data.data);
    ...
    const deleteData = (x) => {
        ...
        Swal.fire({
            title: 'Perhatian!',
            html: '<i>Anda yakin ingin menghapus <b>' + x.jalur_kompetensi + '</b> ?</i>',
            ...
            if (response.isConfirmed) {
                new ServiceApi().deleteBentukKompetensi(data)
                    .then(response => {
                        Swal.fire({
                            title: 'Sukses!',
    ...
    return (
        <div className='main-animation'>
            <div className="d-flex flex-row justify-content-between align-items-center">
                <div>
                    <h3 className="content-title">Bentuk & Jalur Kompetensi</h3>
                </div>
                <div>
                    <Link className="content-link" to={{ pathname: `/master/bentuk_jalur_kompetensi/tambah` }}><Button className="content-button d-flex flex-row align-items-center"><AiIcons.AiOutlinePlus style={style} />&nbsp; Tambah Data</Button></Link>
                </div>
            </div>
                            ...
                            <table className="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th className="table-title" scope="col">Jalur Pengembangan Kompetensi</th>
                                        <th className="table-title text-center" scope="col">Bentuk Kompetensi</th>
                                        <th className="table-title text-center" scope="col">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {
                                        !_.isEmpty(listBentuk) ?
                                            listBentuk.map((x, key) => {
                                                return (
                                                    <tr key={key}>
                                                        <td>{currentPage > 1 ? ((currentPage - 1) * perPage) + key + 1 : key + 1}</td>
                                                        <td>{x.jalur_kompetensi ?? '-'}</td>
                                                        <td className="text-center">{x.bentuk_kompetensi ? x.bentuk_kompetensi == '1' ? 'Klasikal' : 'Non Klasikal' : '-'}</td>
                                                        <td className="action-column">
                                                            <Link to={{ pathname: `/master/bentuk_jalur_kompetensi/detail`, state: { x } }}>
                                                            ...
                                                            <Link to={{ pathname: `/master/bentuk_jalur_kompetensi/edit`, state: { x } }}>
                            ...
                            <div>
                                {
                                    !_.isEmpty(listBentuk) ?
                                        <>
                                            Menampilkan data {((currentPage * perPage) - perPage) + 1} - {listBentuk.length == perPage ? (currentPage * perPage) : (currentPage * perPage) - (perPage - listBentuk.length)} dari {dataCount} data
                                        </>
                                        :
                                        <>
...
export default BentukKompetensi;
